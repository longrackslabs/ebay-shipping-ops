...
def test_schedule_pickup_skips_if_already_scheduled(mock_client_cls, tmp_path):
    """Test that schedule_pickup skips if a pickup is already scheduled for the next pickup date."""
    from ebay_shipper.label_provider import next_pickup_date

    mock_client = MagicMock()
    mock_client_cls.return_value = mock_client

    # Pre-save a pickup state for the next valid pickup date
